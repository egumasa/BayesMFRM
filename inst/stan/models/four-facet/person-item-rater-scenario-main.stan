...
// person, item, rater, scenario
data {
  int<lower=1> J_person;         // # persons
  int<lower=1> J_item;           // # items / criteria
  ...
  array[N] int<lower=1, upper=J_item> item_id;
...
parameters {
  ...
  vector[J_item] beta_raw;           // item difficulty
...
model {
  ...
  for (n in 1:N) {
    ...
    real eta = theta[j] - beta[i] - rater[r] - scenario[s];
...
generated quantities {
  ...
  for (n in 1:N) {
    ...
    {
      real eta_fair = theta[j] - beta[i];  // average rater & scenario = 0
